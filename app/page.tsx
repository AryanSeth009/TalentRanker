    ...
    try {
      ...
      setTimeout(() => {
        ...
        toast({
          ...
          description: `Successfully analyzed ${files.length} resumes. Found ${analysisResults.filter((r: AnalysisResult) => r.score >= 70).length} strong matches.`,
        })
      }, 500)
    } catch (error) {
      clearInterval(progressInterval)
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "There was an error analyzing the resumes.",
        variant: "destructive",
      })
    }
  }
        ...
        {!showResults ? (
          <div className="max-w-4xl mx-auto space-y-8">
            ...
            <div className="text-center space-y-4">
              <h2 className="text-4xl font-bold text-gray-900">Smart Resume Analysis</h2>
              <p className="text-xl text-gray-600 max-w-2xl mx-auto">
                Upload resumes and job description to get AI-powered candidate analysis with detailed scoring and
                insights
              </p>
              ...
              <CardContent className="p-6">
                <div className="space-y-6">
                  <div className="border-2 border-dashed border-blue-300 rounded-xl p-8 text-center hover:border-blue-400 transition-colors bg-blue-50">
                    ...
                    />
                    <p className="mt-3 text-sm text-gray-600">Drag and drop resume files here or click to browse</p>
                    <p className="text-xs text-gray-500 mt-1">
                      For demo: Any PDF/DOCX files will generate realistic mock resume content
                    </p>
                  </div>
                  ...
                  {files.length > 0 && (
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <h4 className="font-semibold text-gray-800">Uploaded Files ({files.length}/20)</h4>
                        <Button
                          variant="outline"
                          size="sm"
                ...
                <CardContent className="p-6">
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-lg font-medium text-gray-900">Analyzing Resumes...</span>
                    </div>
            ...
            <div className="text-center">
              <Button
                ...
                size="lg"
                className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white px-12 py-6 text-xl rounded-xl shadow-lg hover:shadow-xl transition-all duration-300"
              >
                {isAnalyzing ? (
                  <>
            ...
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-gray-600">
                  Analyzed {results.length} candidates • {results.filter((r) => r.score >= 80).length} high matches •{" "}
                  {results.filter((r) => r.score >= 60 && r.score < 80).length} medium matches
                </p>
              </div>
              ...
              <Card className="shadow-lg border-0">
                <CardContent className="pt-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-2xl font-bold text-green-600">{results.filter((r) => r.score >= 80).length}</p>
              ...
              <Card className="shadow-lg border-0">
                <CardContent className="pt-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-2xl font-bold text-red-600">{results.filter((r) => r.score < 60).length}</p>
                    ...
                    <CardContent className="p-6">
                      <div className="grid md:grid-cols-4 gap-6">
                        ...
                        <div className="space-y-3">
                          <div>
                            <h3 className="font-bold text-xl text-gray-900">{result.candidateName}</h3>
                            <p className="text-sm text-gray-600">{result.fileName}</p>
                          </div>
                          <Badge className={`font-bold text-lg px-3 py-1 border ${getScoreBadgeColor(result.score)}`}>
                            {result.score}% Match
                          </Badge>
                        </div>
                        ...
                        {result.skills && result.skills.length > 0 && (
                          <div className="mt-2">
                            ...
                            {result.skills.map((skill, idx) => (
                              <span key={idx} className="inline-block bg-blue-100 text-blue-800 text-xs font-medium mr-1 mb-1 px-2 py-0.5 rounded border border-blue-200">
                                {skill}
                              </span>
                            ))}
                          </div>
